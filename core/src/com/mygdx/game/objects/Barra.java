...
public class Barra extends FiguraGrafica {
    ...
    protected void chocarEnX(float delta) {
        if (this.getPosition().x > -1) {//si la pala esta a la izquierda de la pala la pinta
            ...
            this.getPosition().x =(float)this.widhtT - this.width;
            getPosition().add(getVelocity().cpy().scl(delta));

        }
        if (this.getPosition().x + this.width < this.widhtT) {// si la pala esta a la derecha de la pala la pinta
    ...
    protected void chocarEnY(float delta) {
        ...
        if (this.getY() + this.height < this.heightT) {//si la parte de abajo de la pala es amyor que la altura la pinta
    ...
    protected void cambiarPosicionYSiLimite(float delta) {
        if(this.getPosition().y == 0){
            this.getPosition().y =(float)this.widhtT - this.width;
            getPosition().add(getVelocity().cpy().scl(delta));
        }
    }
